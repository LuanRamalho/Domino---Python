...
def computer_play(auto_player, screen):
    ...
    playedtile_sound = path.join('sounds','playedtile.ogg')
    ...
    if chosen_tile != "PASS" :
        ...
        #If the human didn't say "PASS" last time, set __PASS__ to 1
            ...
            note_sound = path.join('sounds','note.ogg')
...
def check_if_clicked_on_exit(screen, x, y, check_for_popup = 0):
    ...
    if check_for_popup == 1 :
        ...
        replay_sound = path.join('sounds','replay.ogg')
        replay_soundtrack = pygame.mixer.Sound(replay_sound)
        replay_soundtrack.set_volume(0.9)

        cancelReplay_sound = path.join('sounds','cancel_replay.ogg')
...
def END_GAME(screen):
    ...
    if human_score < computer_score :
        ...
        winner_sound = path.join('sounds','game_over_win.ogg')
        ...
        loser_sound = path.join('sounds','game_over_lose.ogg')
        ...
        nonwinner_sound = path.join('sounds','none_winer.ogg')
