...
def initialize(screen):
    ...
    identifier = 0
    x_start = int((main_window_resolution[0] - 565) / 2 + 25)  # Converting to int
    x_end = x_start + 514
    y_start = int(main_window_resolution[1] - (main_window_resolution[1] / 8) - 34)  # Converting to int
    ...
    identifier = 0
    y_start = int(main_window_resolution[1] / 8) - 34  # Converting to int

    for i in range(x_start, x_end, 80):
        COMPUTER_TILES.append([both_players_tiles[1][identifier], (i, y_start)])
        identifier += 1

    # Draw the background and domino tiles on the screen
    draw_bg(screen)
    draw_tiles(screen)
